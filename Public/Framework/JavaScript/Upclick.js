    ...
    this.Create = function()
    {
        ...
        i.onload_callback = function()
        {
            ...
            var o = function()
            {
                ...
                {                 
                    if (d.type.indexOf(/[^.]+$/.exec(b.files[0].name)[0]) === -1) return FILE_EXT_INVALID;
                    if (b.files[0].size > d.maxsize) return FILE_TOO_BIG;    
                    return true;
                };    
                if(b.value)
                {
                    switch (this.CheckFile()) 
                    {
                        case FILE_TOO_BIG:
                        {
                            alert('File too big. Allow size <= '+d.maxsize+' bytes');
                            return false;
                        }
                        case FILE_EXT_INVALID:
                        {
                            alert('Extension file invalid. Allow: '+d.type);
                            return false;
                        }
                    }
                    var f=d.onstart;f&&f(b.value);a.submit();
                }
            };
            if (b.addEventListener) b.addEventListener("change", o, false);
            else if (b.attachEvent) b.attachEvent
            (
                "onpropertychange", 
                function(f) 
                {
        ...
        g = function(a)
        {
            ...
            else if(a.clientY) y = a.clientY + (e.documentElement.scrollTop ? cumentElement.scrollTop: e.body.scrollTop);
